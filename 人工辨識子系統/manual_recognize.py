...
def filter_accidents():
    ...
    # 初始化查詢語句
    query = "SELECT accident_id, location, date_time, recognized FROM Accidents WHERE 1=1"
    params = []
    
    # 關鍵字篩選
    if keyword:
        query += " AND (location LIKE %s OR accident_id LIKE %s)"
        params.extend([f"%{keyword}%", f"%{keyword}%"])
    
    # 狀態篩選
    if status:
        if status == '已辨識':
            # 包含 recognized = 2 和 recognized = 3 的事故
            query += " AND recognized IN (2, 3)"
        elif status == '未辨識':
            query += " AND recognized = 1"
        elif status == '無法辨識':
            query += " AND recognized = 5"
